...
var newArray = liste.concat(liste2);

	var newArray =[i]
		for(int i=0; i<= newArray.lenght-1; i++){
			for(int x= 0; x<= newArray.lenght-1; i++){
				int element = monTableau[i];
				document.getElementById('test').innerHTML+='<td>'+i+'</td>'
		}
}
